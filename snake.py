...
class Snake:
    ...
    def eating_apple(self):
        if self.head.pos == self.apple.pos and not self.is_virtual_snake and not self.won_game:
            ...
            self.moves_without_eating = 0
            self.score += 1
            pygame.mixer.Sound(r"eat.mp3").play()
            return True
    ...
    def bfs(self, s, e):  #the shortest path between (start_position, end_position)
        q = [s]
        visited = {tuple(pos): False for pos in GRID}#dictionary that contains visited positions
        visited[s] = True
        prev = {tuple(pos): None for pos in GRID}# Prev to find the parent node of each node
    ...
    def set_path(self):
        ...
        if self.longest_path_to_tail() and self.score % 2 == 0 and self.moves_without_eating < MAX_MOVES_WITHOUT_EATING / 2:
